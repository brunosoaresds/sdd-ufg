		...
		if ($clazz_id != null && $teacher_id != null) {
			...
			if ($this->RequestHandler->accepts('ajax')) {
				...
				if ($teacher_id != null && $clazz_id != null && $allocate == 'allocate') {
					...
					$clazzTeachers = $table_clazzes_teachers->find('all')->where(['clazz_id' => $clazz_id, 'teacher_id' => $teacher_id])->toArray(); 
					$hasInterest = false;
					
					if ($clazzTeachers) {
						if ($clazzTeachers[0]->previous_status != 'NO_INTEREST') {
							$hasInterest = true;
						}
					}
					...
					$query = $table_clazzes_teachers->query();
					$query->insert(['clazz_id', 'teacher_id', 'status', 'previous_status'])->values([
							'clazz_id' => $clazz_id,
							'teacher_id' => $teacher_id,
							'status' => 'SELECTED',
							'previous_status' => ($hasInterest ? 'PENDING' : 'NO_INTEREST')
						])->execute();
